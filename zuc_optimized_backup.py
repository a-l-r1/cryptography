...
def gen_key_iter_infinite(k: bytes, iv: bytes) -> Iterable[int]:
    ...
    def _l1(x: int) -> int:
        return x ^ _32_bit_left_rotate(x, 2) ^ _32_bit_left_rotate(x, 10) ^ \
            _32_bit_left_rotate(x, 18) ^ _32_bit_left_rotate(x, 24)

    def _l2(x: int) -> int:
        return x ^ _32_bit_left_rotate(x, 8) ^ _32_bit_left_rotate(x, 14) ^ \
            _32_bit_left_rotate(x, 22) ^ _32_bit_left_rotate(x, 30)

    def _lfsr(index: int) -> int:
        nonlocal lfsr, lfsr_curr_ptr
        return lfsr[(lfsr_curr_ptr + index) % _lfsr_length]

    def _bit_reconstruction() -> Tuple[int, int, int, int]:
        nonlocal lfsr, lfsr_curr_ptr
        ...
        return _get_h_l(15, 14), _get_l_h(11, 9), _get_l_h(7, 5), _get_l_h(2, 0)

    def _f(x0_: int, x1_: int, x2_: int) -> int:
        nonlocal r1, r2

        def _get_l_h(a: int, b: int) -> int:
            _high_bitmask = 0xffff0000
            _low_bitmask = 0x0000ffff

            return ((a & _low_bitmask) << 16) | ((b & _high_bitmask) >> 16)

        w_ = ((x0_ ^ r1) + r2) & _2_power_32_minus_1
        w1 = (r1 + x1_) & _2_power_32_minus_1
        w2 = r2 ^ x2_

        r1 = _s(_l1(_get_l_h(w1, w2)))
        r2 = _s(_l2(_get_l_h(w2, w1)))
    ...
    for _ in range(_init_phase_round_count):
        x0, x1, x2, x3 = _bit_reconstruction()
        w = _f(x0, x1, x2)
        _lfsr_with_initialization_mode(w >> 1)
